...
def gen_arti(data_type=0,nbex=1000,centerx=1,centery=1,sigma=0.1,epsilon=0.01):
         ...
         if data_type==1:
             ...
             xpos=np.vstack((np.random.multivariate_normal([centerx,centery],np.diag([sigma,sigma]),nbex/4),np.random.multivariate_normal([-centerx,-centerx],np.diag([sigma,sigma]),nbex/4)))
             xneg=np.vstack((np.random.multivariate_normal([-centerx,centery],np.diag([sigma,sigma]),nbex/4),np.random.multivariate_normal([centerx,-centerx],np.diag([sigma,sigma]),nbex/4)))
             data=np.vstack((xpos,xneg))
             y=np.hstack((np.ones(nbex/2),-np.ones(nbex/2)))
...
#	#print 'vecteur label -1', x[labels==-1,0]	
#	l=(labels<0).astype(int)
#	plt.scatter(x[l,0],x[l ,1],c='red',marker='x')
#	l2=(labels>0).astype(int)
#	plt.scatter(x[l2,0],x[l2,1],c='green',marker='+')
